...
export default function FundraisingPage() {
  return (
    <div className="section">
      ...
      <p style={{ color: 'var(--muted)', fontSize: '0.85rem', lineHeight: 1.7, maxWidth: '600px', marginBottom: '3rem' }}>
      ...
      <div className="grid-3">
        {FUNDRAISERS.map(f => (
          <div key={f.id} className="card-gold-left">
            ...
            <p style={{ color: 'var(--muted)', fontSize: '0.88rem', lineHeight: 1.7, marginBottom: '1.2rem' }}>{f.desc}</p>
      ...
      <div style={{ marginTop: '4rem', background: 'linear-gradient(135deg,rgba(232,160,0,0.08),rgba(232,160,0,0.02))', border: '1px solid rgba(232,160,0,0.2)', padding: '4rem 2rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ color: 'var(--muted)', fontFamily: "'Barlow Condensed',sans-serif", fontSize: '1rem', marginTop: '0.5rem' }}>Updated live at the arena · Winner drawn after Gold Medal Game</p>
        <p style={{ color: 'var(--muted)', fontFamily: "'Barlow Condensed',sans-serif", fontSize: '0.8rem', marginTop: '1rem', opacity: 0.6 }}>Must be present to win · 50% to winner · 50% to Cardinals program</p>
      </div>
    </div>
  );
}

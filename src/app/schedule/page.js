          ...
          return (
            <div key={key}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '1rem', marginBottom: '1rem', paddingBottom: '0.8rem', borderBottom: '1px solid #1a1a1a' }}>
                ...
                <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: '0.8rem', letterSpacing: '2px', color: 'var(--muted)', textTransform: 'uppercase' }}>{sub}</div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: '#1a1a1a' }}>
                {dayGames.map(g => {
                  ...
                  return (
                    <div key={g.id} style={{ background: '#0a0a0a', padding: '1rem 1.5rem', display: 'grid', gridTemplateColumns: '80px 1fr auto 1fr auto', alignItems: 'center', gap: '1rem' }}>
                      ...
                      <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '1.1rem', color: 'var(--muted)', textAlign: 'center' }}>VS</div>
                      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700, color: g.awayTeamId === 'cardinals' ? '#FFB800' : '#F5F0E8' }}>{away.name}</div>
                      <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: '0.75rem', color: 'var(--muted)', textAlign: 'right' }}>
                        {g.label || g.gameType}
                      </div>
                    </div>
                  );
                ...
                {dayGames.length === 0 && (
                  <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--muted)', fontFamily: "'Barlow Condensed',sans-serif", letterSpacing: '2px' }}>Schedule TBD</div>
                )}
              </div>
            </div>
          );
